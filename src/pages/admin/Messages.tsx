...
import { useAdminMessages, useUpdateMessageStatus, useDeleteMessage, useMessageReplies, useCreateReply } from '../../hooks/useMessages'
...
function MessageRow({ msg }: { msg: Message }) {
  ...
  const deleteReply = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['message-replies', msg.id] }); showToast('回复已删除') },
  })

  const handleStatusChange = (status: string) => {
    updateStatus.mutate({ id: msg.id, status }, {
  ...
  const handleOfficialReply = () => {
    ...
    createReply.mutate({ messageId: msg.id, content: replyText.trim(), isOfficial: true }, {
      onSuccess: () => { setReplyText(''); showToast('官方回复已发布') },
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => setDeleteOpen(true)} className="text-xs text-red-400 hover:text-red-500 flex items-center gap-1 ml-auto transition-colors">
              <Trash2 size={12} /> 删除
            </button>
          </div>
        </div>
        ...
        <div className="mt-3 pt-3 border-t border-gray-50">
          {replies?.map(reply => (
            <div key={reply.id} className={`mb-2 flex items-start gap-2 pl-3 border-l-2 ${reply.is_official ? 'border-emerald-400' : 'border-gray-200'}`}>
              ...
              <button onClick={() => deleteReply.mutate(reply.id)} className="text-gray-300 hover:text-red-400 transition-colors shrink-0 mt-0.5">
                <Trash2 size={12} />
              </button>
            </div>
          ))}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => deleteMsg.mutate(msg.id, { onSuccess: () => setDeleteOpen(false), onError: () => showToast('删除失败', 'error') })}
        message={`确定要删除该留言吗？`}
...
export default function AdminMessages() {
  ...
  const { data: messages, isLoading } = useAdminMessages({ status: tab === 'all' ? undefined : tab, search })
